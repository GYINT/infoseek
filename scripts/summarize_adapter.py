...
SUMMARIZE_CACHE = WORKSPACE / 'infoseek' / 'summarize_cache.json'
...
def _dual_run(text: str, max_words: int = 100) -> dict:
    ...
    if result.get('method') not in ('summa_textrank', 'jieba_textrank_zh', 'regex_wordfreq_en'):
    ...
    if prefer == "llm" and llm_api_key:
        ...
        if result is None:
            ...
            if result is None:
                ...
                result["fallback_reason"] = "llm_failed + summa_unavailable + jieba_unavailable"
            else:
        ...
        if result is None:
            ...
            if result is None:
                result = _truncation_fallback(text)
                result["fallback_reason"] = "jieba_unavailable + summa_unavailable"
            else:
        ...
        if result is None:
            ...
            if result is None:
                fallback_used = True
                fallback_reason = "summa_unavailable + jieba_unavailable"
                result = _truncation_fallback(text)
            else:
                # summa 不可用但 jieba 可用
                fallback_used = True
