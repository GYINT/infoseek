#!/usr/bin/env python3
"""infoseek_mcp_server.py — Infoseek v1.7.0 MCP 服务器（search server）

版本演进:
  v1.5.0: stdio 传输，6 工具
  v1.5.1: + SSE 传输 + Bearer Token 认证
  v1.5.2: + HTTP /rpc + GET /health
  v1.6.0: + cross_subject_analysis 第 7 工具 + multi-server 拆分（archive 独立）
  v1.6.1: PATCH 加固: token 来源诊断横幅 + 错误响应含 hint + token 脱敏日志
  v1.6.2: PATCH 增强: 健康检查细化（uptime + 工具调用统计）+ 审计日志 + token 健康端点
  v1.7.0: MINOR 新增: summarize_content 第 8 工具（summa 主路径 + LLM 兜底）
...
工具（v1.6.0 起 7 个）: search_anchors / fetch_content / save_archive /
...
INFOSEEK_DIR = WORKSPACE / 'infoseek'
DB_PATH = INFOSEEK_DIR / 'infoseek_db.json'
LOG_PATH = INFOSEEK_DIR / 'infoseek.log'
...
AUDIT_LOG_PATH = WORKSPACE / 'infoseek' / 'audit.log'
...
TOOLS = [
    ...
    {
        "name": "research_v3",
        "description": "v3.0.0 GA 异步研究（async_research 包装）。first yield 4ms 优化已下沉到 streaming_research，此处返回一次性完整结果。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject": {"type": "string", "description": "调研主题（必填）"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 流式研究（7 步 yield 同步收集）。同步 JSON-RPC 接口下需收集所有 yield，首步优势需 SSE 客户端。",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject": {"type": "string", "description": "调研主题（必填）"},
    ...
    # 11 个 async 工具（backend 与 TOOLS list 均已注册）
    {
        "name": "search_anchors_async",
        "description": "v3.0.0 GA 异步版 search_anchors（asyncio.to_thread 包装）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 fetch_content",
        "inputSchema": {
            "type": "object",
            "properties": {
                "url": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 save_archive（v1.4.0 增强）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject": {"type": "string", "description": "调研主题（必填）"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 check_dedup",
        "inputSchema": {
            "type": "object",
            "properties": {
                "url": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 dedup_stats",
        "inputSchema": {"type": "object", "properties": {}}
    },
    {
        "name": "fuse_analysis_async",
        "description": "v3.0.0 GA 异步版 fuse_analysis",
        "inputSchema": {
            "type": "object",
            "properties": {
                "subject": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 cross_subject_analysis",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 summarize_content（DeepSeek 真实 LLM 路径）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 conflict_detection",
        "inputSchema": {
            "type": "object",
            "properties": {
                "sources": {"type": "array", "items": {"type": "object"}},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 score_source",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {"type": "string"},
    ...
    },
    {
        ...
        "description": "v3.0.0 GA 异步版 score_contradiction（v2.7.2 引入）",
        "inputSchema": {
            "type": "object",
            "properties": {
                "claim_a": {"type": "object"},
...
def handle_tools_call(req_id: int, params: Dict) -> Dict:
    ...
    try:
        ...
        # 新增 async 工具 + research_stream（向后兼容：旧工具保留）
...
def run_sse_server(port: int, require_token: bool, fixed_token: Optional[str]):
    ...
    class SSEHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            """POST /messages 或 POST /rpc — 接收 client-to-server JSON-RPC 消息"""
...
# research_v3 + research_stream 工具函数
def _handle_async_research_wrapper(args: Dict) -> Dict:
    ...
    # v3.0.0 GA: 顶层 version 覆盖为 3.0.0（MCP server 标识）
    if isinstance(result, dict):
...
async def _stream_research_wrapper(args: Dict):
    """research_stream async generator"""
    sys.path.insert(0, str(Path(__file__).parent))
...
def _handle_research_stream_sync(args: Dict, max_steps: int = 10) -> List[Dict]:
    """research_stream 同步收集（截断到 max_steps 步）"""
    import asyncio
    parts = []
    async def _collect():
        count = 0
